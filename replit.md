...
- **Complete Supabase Migration (Oct 17):** Migrated all database operations from mixed pg/Supabase to **pure Supabase-only** architecture for complete consistency. Removed React `cache()` wrapper from `getUserDetails` to prevent stale data. All avatar updates, profile reads, and review queries now use Supabase service-role client with proper JOINs to bypass RLS policies. Uses `revalidatePath()` to clear Next.js cache. Avatars and profile changes now persist correctly across all pages. Comments dynamically display current user avatar/name via JOIN - when users update their profile, all their past comments automatically show the new information in real-time.
