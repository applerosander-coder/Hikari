...
- **Seller UX Enhancements:** Added location detection button (MapPin icon) next to location field that uses browser geolocation API and reverse geocoding to suggest user's current city/state. Added "Now" checkbox next to Start Date that automatically fills in current date and time, unchecks when manually changed.
