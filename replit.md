...
- **Two-Tier Auction & Category Navigation:** Replaced single-row pill chip filters with a professional two-tier micro-nav system across Dashboard and Leaderboard pages. Top row displays auction scopes as compact rounded tabs (h-8, text-sm) with black background for active tab. Second row shows category chips as horizontally scrollable pills with white background and black border for selected category. Both rows contained in sticky container below safe-area inset with blurred white/80 background. Implements intelligent overflow handling with "More ▼" button that opens bottom sheet displaying all options in two columns. Created reusable AuctionCategoryNav component in components/ accepting auction/category options as props with independent state management for both filter types. Mobile-optimized horizontal scrolling with hidden scrollbar, respects iPhone safe-area-inset-top for edge-to-edge layout.
- **Landing Page Confetti Effect:** Added CoolMode particle animation to "More Confetti" button on hero section. When users touch/hold the button, gold/silver/white confetti particles (matching background glitter style) shoot out in random directions with sparkle effects. Mix of circles and squares for variety, same aesthetic as "Get Auction Alerts" button. Button is purely decorative (no redirect), includes user-select-none to prevent text highlighting on mobile during press-and-hold interaction.
- **Dashboard Pill Chip Filters:** Replaced two stacked dropdown filters with a single horizontal row of pill chips for a cleaner, more modern filtering experience. Pills include "All Items" (combines auctions + categories), individual category pills (Electronics, Fashion, etc. - only showing categories with items), and auction pills (live auctions first, then 3 most recent ended). Active pill has primary background, inactive pills use muted background. Horizontal scrollable row with hidden scrollbar for mobile-friendly navigation. Clicking any pill auto-resets the other filter type to "all" for intuitive UX. Applied same pill chip design to Leaderboard page filters for consistent UX across dashboard. Removed subtitle text below Leaderboard and My Bids page titles for cleaner appearance.
- **Compact Navigation Bars:** Reduced height and padding across all navigation components for a more compact, modern appearance while maintaining iPhone notch safety. Marketing navigation: reduced from h-12 to h-9 logo, mt-4 to mt-1 margin, smaller padding (p-1.5 from p-2). Dashboard mobile navbar: reduced from h-20 to h-14 height, h-10 to h-8 logo, smaller gap/padding. Content area adjusted from pt-20 to pt-14 to match new navbar height. All changes preserve safe area insets for proper iPhone edge-to-edge display. Added dynamic page title support to mobile navbar - displays contextual page titles ("Auctions", "My Bids", "Leaderboard", "Seller Dashboard") centered between logo and hamburger menu based on current page. Page content h1 headings hidden on mobile to prevent duplication, desktop view unchanged.
