...
- **Bidding System Database Fix:** Resolved critical bidding failure caused by legacy database trigger. The `notify_outbid()` trigger function was using outdated column references (`amount` instead of `bid_amount`, non-existent `full_name` and `auction_item_id` columns). Updated trigger to use correct schema: `bid_amount` for sorting, email addresses from `auth.users`, and proper notification fields (user_id, type, title, message). Bidding now works correctly with outbid notifications sent to previous highest bidders.
