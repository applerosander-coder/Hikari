...
- **Dashboard Pill Chip Filters:** Replaced two stacked dropdown filters with a single horizontal row of pill chips for a cleaner, more modern filtering experience. Pills show "All Items" and auction pills only (live auctions first, then 3 most recent ended). Active pill has primary background, inactive pills use muted background. Horizontal scrollable row with hidden scrollbar for mobile-friendly navigation. Category filtering moved to a dedicated dropdown button (Filter icon) positioned next to the search bar for cleaner organization. Clicking any pill auto-resets category filter to "all" for intuitive UX. Applied same pill chip design to Leaderboard page filters for consistent UX across dashboard. Removed subtitle text below Leaderboard and My Bids page titles for cleaner appearance.
