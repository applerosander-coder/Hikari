...
- **Dashboard Pill Chip Filters:** Replaced two stacked dropdown filters with a single horizontal row of pill chips for a cleaner, more modern filtering experience. First pill renamed to "Auction Items" (from "All Items"). Auction pills show creator avatar (20x20px circular image) to the left of auction name when available, pulled from users table via created_by relationship. Pills show live auctions first, then 3 most recent ended auctions. Active pill has primary background, inactive pills use muted background. Horizontal scrollable row with hidden scrollbar for mobile-friendly navigation. Category filtering moved to a dedicated dropdown button (Filter icon) positioned next to the search bar for cleaner organization. Clicking any pill auto-resets category filter to "all" for intuitive UX. Applied same pill chip design to Leaderboard page - renamed to "All Auction Items", added creator avatars to auction pills, moved categories to Filter dropdown button next to sort controls. Updated users table RLS policy to allow all authenticated users to SELECT (view) any user's profile data for cross-user avatar visibility, while keeping UPDATE restricted to own data only. Removed subtitle text below Leaderboard and My Bids page titles for cleaner appearance.
