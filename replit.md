...
- **Auction Delete Functionality:** Added delete button with confirmation dialog to seller page. Users can now delete their draft auctions directly from the auction list. The delete button (trash icon) appears next to the draft toggle. Clicking it opens a confirmation dialog asking "Are you sure you want to delete this auction?" with warning that the action cannot be undone. The API endpoint handles complete cleanup: deletes auction items, removes images from storage, and deletes the auction record. Only draft auctions can be deleted for safety.
- **Fixed My Bids Active Tab Filtering:** Updated My Bids page to only show items from auctions with status='active' in the Active, Outbid, Ending Soon, and Watchlist tabs. Previously, items from draft auctions were incorrectly displayed. The filters now check `status === 'active'` instead of `status !== 'ended'`, ensuring only truly active auction items appear.
- **Admin API Endpoint:** Created secure `/api/admin/users` endpoint to retrieve all registered user emails. Protected with ADMIN_SECRET bearer token authentication. Uses Supabase admin client with service role key to access auth.users data. Returns user details including id, email, created_at, confirmed_at, last_sign_in_at, and user_metadata.
- **Ended Tab Toggle for Won/Lost Auctions:** Added toggle buttons in the Ended tab of My Bids page to switch between viewing won auctions and lost auctions separately. Users can click "Won (count)" or "Lost (count)" buttons to filter their ended auctions, providing a cleaner, more focused view of their bidding history.
- **Sign-Out Confirmation Dialog:** Added confirmation dialog before users sign out from both mobile menu and desktop sidebar. When clicking "Sign Out", users now see a dialog asking "Are you sure you want to sign out?" with options to Cancel or confirm. This prevents accidental sign-outs and improves user experience.
- **Updated BIDWIN Logo Across All Pages:** Replaced all gavel icons with custom BIDWIN logo that automatically adapts to light and dark modes. The logo displays black gavel/text in light mode and white outlined gavel/text in dark mode. Files optimized to 11KB (light) and 26KB (dark) for optimal performance. Updated locations:
