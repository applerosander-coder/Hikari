...
- **Edit Draft Auction Form Parity:** Updated edit draft auction form to match create auction form exactly - same structure, features, and layout. Now includes AI description generator with auto-category selection, location detection button, "Now" checkbox for start date, "24h" checkbox for end date, and proper field ordering (Title → Image → Description with AI button → Category → Prices). Both forms now provide identical seller experience.
...
- **Seller Form Layout Improvement:** Reorganized auction item creation form to display image upload immediately after item title for better visual flow. New order: Title → Image → Description (with AI button) → Category → Prices. This creates a more intuitive workflow where users can see their uploaded image right after entering the title. Added glassmorphism-style inner glow effect to AI "Type for me" button with very slow, smooth linear animation (30-second cycle) featuring a prominent silver ambient glow that gradually expands and contracts to draw attention without being distracting. The button only activates its electrical sparkle glow effect after an image is uploaded, creating a visual reward and encouraging sellers to use the AI description generator with product photos.
- **AI-Powered Description Generator:** Added "Type for me" button on seller page that uses OpenAI's Vision API (GPT-4o) to automatically generate compelling product descriptions and suggest categories. The feature works flexibly with item title only, image only, or both together - analyzing available inputs to create engaging 50-100 word descriptions highlighting key features, materials, colors, and selling points, while also automatically selecting the most appropriate category from available options. Uses GPT-4o model for fast, cost-effective generation (switched from GPT-5 for speed and cost efficiency). Button includes Sparkles icon, loading state with spinner, and is disabled only when both image and title are missing. API endpoint is secured with Supabase authentication to prevent unauthorized access. Provides instant AI assistance to sellers for creating professional auction listings without requiring all fields. Implements one-click-per-photo limit: AI button can only be clicked once per uploaded image, then resets when a new photo is uploaded to prevent excessive API usage.
