...
- **Dashboard Two-Part Toolbar:** Replaced horizontal pill chip bar with a compact two-part sticky toolbar (h-9, text-sm). Left pill shows auction scope with dropdown menu (displays "Items (count)" for all, or specific auction name with count). Right pill opens bottom sheet for category selection (70vh height with safe-bottom padding). Toolbar uses sticky positioning at 56px on mobile (directly below navbar) and top-0 on desktop, with z-30 layering below navbar (z-40) but above content. Dropdown shows all auctions with metadata (place, item count, ended status). Bottom sheet displays all categories as full-width buttons. Selecting auction/category auto-resets the other filter to "all" for intuitive UX. Text truncation on pills (max-w-[120px] mobile, max-w-[200px] desktop) ensures compact appearance. Navbar increased to z-40 to stay above all dropdowns.
