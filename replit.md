...
- **Avatar Persistence Fix (Oct 17):** Migrated avatar updates from mixed pg/Supabase approach to pure Supabase service client. Removed React `cache()` wrapper from `getUserDetails` to prevent stale data. Avatar updates now use Supabase service-role client with upsert to ensure consistent reads/writes. Uses `revalidatePath()` to clear Next.js cache. Avatars now persist correctly across navigation and page refreshes. Comments now dynamically display current user avatar/name instead of historical data - when users update their profile, all their past comments automatically show the new information.
