  ...
  try {
      ...
      .insert([
        {
          ...
          description: 'Contemporary artwork by emerging artist',
        ...
        },
        {
          ...
          description: 'Rare 1950s cameras',
    ...
    const { data: auction2, error: auction2Error } = await supabase
      ...
      .insert({
        ...
        start_date: new Date().toISOString(),
        end_date: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(), // 3 hours from now
        created_by: user.id,
        status: 'active'
      })
      .select()
      ...
      .insert([
        {
          ...
          description: 'Classic consoles with games',
        ...
        },
        {
          ...
          description: 'Hi-Fi audio equipment',
    ...
    // Set winner_id on some ENDED auction items to populate Won tab
    ...
    if (endedItems && endedItems.length > 0) {
      const itemsToWin = endedItems.slice(0, 2); // Win 2 items
      const { error: winError } = await supabase
        .from('auction_items')
        .update({ winner_id: user.id })
        .in('id', itemsToWin.map(item => item.id));

      if (winError) console.error('Win error:', winError);
    }

    // Get the new live auction items for watchlist
    ...
    return NextResponse.json({ 
      success: true, 
      message: `Created 2 live auctions with 5 items, set ${endedItems?.length || 0} won items, added ${newItems?.length || 0} to watchlist` 
    });
